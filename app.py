...
def index():
    ...
    faturamento = db.ver_faturamento()
...
def produto_novo():
    if request.method == "POST":
        ...
        if erro_preco:
            ...
            ok, erro = db.cadastrar_produto(nome, preco, estoque)
            if ok:
...
def produto_editar(produto_id):
    ...
    if request.method == "POST":
        ...
        if erro_preco:
            ...
            ok, erro = db.atualizar_produto(produto_id, nome, preco, estoque)
            if ok:
...
def produto_excluir(produto_id):
    ok, erro = db.excluir_produto(produto_id)
    flash("Produto excluído com sucesso." if ok else erro, "sucesso" if ok else "erro")
    return redirect(url_for("produtos"))
...
def estoque():
    if request.method == "POST":
        ...
        if erro_id or produto_id is None:
            ...
            ok, erro, novo_estoque = db.entrada_estoque(produto_id, quantidade)
            flash(f"Entrada registrada! Novo estoque: {novo_estoque}" if ok else erro,
                  "sucesso" if ok else "erro")
        elif tipo == "saida":
            ok, erro, novo_estoque = db.saida_estoque(produto_id, quantidade)
            flash(f"Saída registrada! Novo estoque: {novo_estoque}" if ok else erro,
                  "sucesso" if ok else "erro")
...
def vendas():
    if request.method == "POST":
        ...
        if erro_id or produto_id is None:
            ...
            ok, erro, valor_total = db.registrar_venda(produto_id, quantidade)
            if ok:
